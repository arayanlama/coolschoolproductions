...
if(heroTrack){
  ...
  const start=()=>{clearInterval(timer);if(!window.matchMedia('(prefers-reduced-motion: reduce)').matches)timer=setInterval(()=>goTo(current+1),4000)};
...
function loadAudio(i){audioIndex=(i+audioIds.length)%audioIds.length;const art=`url('https://i.ytimg.com/vi/${audioIds[audioIndex]}/maxresdefault.jpg')`;audioArt.style.backgroundImage=art;if(vinylAtmosphere){vinylAtmosphere.style.opacity='0';setTimeout(()=>{vinylAtmosphere.style.backgroundImage=art;vinylAtmosphere.style.opacity=''},180)}if(audioKicker)audioKicker.textContent='NOW PLAYING';if(audioTitle)audioTitle.textContent=audioArtists[audioIndex];if(audioSubtitle)audioSubtitle.textContent=`COOL SCHOOL PRODUCTIONS · CSP—00${audioIndex+1}`;audioCount.textContent=`0${audioIndex+1} / 03`;audioRecord?.setAttribute('aria-label',`Play ${audioArtists[audioIndex]}`);audioPlay?.setAttribute('aria-label',`Play ${audioArtists[audioIndex]}`);if(audioPlayer?.cueVideoById)audioPlayer.cueVideoById(audioIds[audioIndex])}
